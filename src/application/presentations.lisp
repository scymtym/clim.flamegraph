...
(clim:define-presentation-method clim:present ((object t)
                                               ...
                                               (view   run-list-view)
                                               &key)
  (let ((duration (model::duration object))
        (threads  (model::threads object))
        (traces   (model::traces object)))
    (flet ((do-it (stream)
             ...
             (clim:with-drawing-options (stream :text-face :italic :text-size :smaller)
               (cond ((emptyp traces)
                      (format stream "no traces~%recorded"))
                     (t
                      (format stream "~5,F seconds~%"  duration)
                      (format stream "~:D thread~:P~%" (length threads))
                      (format stream "~:D trace~:P"    (length traces)))))))
